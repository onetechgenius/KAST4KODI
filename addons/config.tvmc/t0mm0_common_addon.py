...
class Addon:
    ...
    def show_countdown(self, time_to_wait, title='', text=''):
        ...
        while secs <= time_to_wait:
            ...
            remaining_display = (
                                'Wait %d seconds for the ' + 'video stream to activate...') % secs_left
            dialog.update(percent, text, remaining_display)
            secs += 1
        if cancelled == True:
            self.log_notice('countdown cancelled'); return False
        else:
            self.log_debug('countdown finished waiting'); return True
    ...
    def add_item(self, queries, infolabels, contextmenu_items='', context_replace=False, img='',
        ...
        if contextmenu_items: listitem.addContextMenuItems(contextmenu_items,
        ...
        if playlist is not False:
            ...
            xbmcplugin.addDirectoryItem(self.handle, play, listitem, isFolder=is_folder,
    ...
    def save_data(self, filename, data):
        ...
        try:
            pickle.dump(data, open(save_path, 'wb')); return True
    ...
    def load_data(self, filename):
        ...
        print profile_path
        if not os.path.isfile(load_path): self.log_debug(
            '%s does not exist' % load_path); return False
        try:
